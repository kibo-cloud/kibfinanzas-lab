> **Esto es kibFinanzas Lab, el taller de pruebas.** No es la app para usar con tus datos de verdad: esa vive en `kibo-cloud.github.io/KF-kibfinanzas/`. La Lab guarda todo aparte (datos con prefijo `kibolab.`, caché `kibfinanzas-lab-`, carpeta interna `copias-lab`) para no tocar nunca la app real, aunque las dos estén instaladas en el mismo teléfono. Para probar con datos reales: hacé una copia en la app real y restaurala acá.
...
L7.1 · 2026-09-11 — Modo Simple: productos rápidos como botones (una venta con un solo toque, con Deshacer) y costo opcional por producto para ver la ganancia del día. Ingresos por semana o quincena con la fuente (Rappi, Uber, changas), y sección "De dónde viene" con lo cobrado de cada una este mes. El CSV suma de dónde, período, costo unitario y ganancia.
L7 · 2026-09-11 — Ritmo del mes en Mes: cuánto te alcanza por día y aviso en rojo si a este ritmo no llegás. En Trabajo: Cobranza de la semana por cliente con "Recordar" (mensaje listo para mandar por el menú de compartir), número de factura sugerido, abonos que se generan solos cada mes y, en Simple, vuelve "Hoy vendiste" con efectivo y transferencias separados. Arreglado: después de pasar por Trabajo, los números de arriba de Mes no se actualizaban mientras escribías.
